...
def build_act(q_func, ob_space, ac_space, stochastic_ph, update_eps_ph, sess, obs_phs=None):
    """
    ...
    _act = tf_util.function(inputs=[policy.obs_ph, stochastic_ph, update_eps_ph, mask],
...
def build_act_with_param_noise(q_func, ob_space, ac_space, stochastic_ph, update_eps_ph, sess,
                               param_noise_filter_func=None, obs_phs=None):
    """
...
def build_train(q_func, ob_space, ac_space, optimizer, sess, grad_norm_clipping=None,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        obs_phs = observation_input(ob_space, batch_size=None, scale=False)
        if param_noise:
            act_f, obs_phs = build_act_with_param_noise(q_func, ob_space, ac_space, stochastic_ph, update_eps_ph, sess,
                                                        param_noise_filter_func=param_noise_filter_func)
        else:
            act_f, obs_phs = build_act(q_func, ob_space, ac_space, stochastic_ph, update_eps_ph, sess, obs_phs=obs_phs)
        ...
        with tf.variable_scope("target_q_func", reuse=False):
            target_policy = q_func(sess, ob_space, ac_space, 1, 1, None, reuse=False, obs_phs=obs_phs)
        ...
        if double_q:
            with tf.variable_scope("double_q", reuse=True, custom_getter=tf_util.outer_scope_getter("double_q")):
                double_policy = q_func(sess, ob_space, ac_space, 1, 1, None, reuse=True, obs_phs=obs_phs)
