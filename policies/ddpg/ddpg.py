...
from policies.ddpg.policies import DDPGPolicy
...
class DDPG(OffPolicyRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("input", reuse=False):
                    ...
                    obs_phs = observation_input(self.observation_space, None, scale=False)


                    self.policy_tf = self.policy(self.sess, self.observation_space, self.action_space, 1, 1, None,
                                                 obs_phs=obs_phs, **self.policy_kwargs)
                    ...
                    if self.param_noise is not None:
                        ...
                        self.param_noise_actor = self.policy(self.sess, self.observation_space, self.action_space, 1, 1,
                                                             None, obs_phs=obs_phs, **self.policy_kwargs)
    ...
    def _policy(self, obs, apply_noise=True, compute_q=True):
        ...
        obs = np.array([np.concatenate(([obs[key] for key in ['day_vec', 'board_config', 'prev_sales']]),
                             axis=None)])
